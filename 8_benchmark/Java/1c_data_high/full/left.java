...
public class left {
	...
	public static void main(String[] args) throws IOException {
		in = new BufferedReader(new FileReader("t.in"));
		out = new PrintWriter(new FileWriter("t.out"));
		...
		for (int t=1; t<=tt; t++) {
			...
			for (int i=0; i<nPpl; i++) {
				...
				long trip = cost(from, to, nStops) % MOD;
				cost += (trip * pass);
				cost %= MOD;
			...
			for (Entry<Long, Long> e : stops.entrySet()) {
				...
				while (numP < 0) {
					...
					long trip = cost(nextS, station, nStops) % MOD;
					better += (trip * pass);
					better %= MOD;
	...
	public static long cost(long a, long b, long nSt) {
		long diff = b-a;
		return (2*nSt - diff + 1) * diff/2;
	}
}
