...
# Get all the recalls
recall.all <- read.csv("~/Dropbox/Working Copies/ICSE 2015/func-clones/study-results/allRecalls.csv", sep=";", dec=".")
...
category.full <- read.csv("../../analyzers/undetectedSampler/full-with-categories.csv", sep=";")
category.full <- cbind(category.full[,6:10], category.full[,1])
colnames(category.full)[6] <- "lang"
category.partial <- read.csv("../../analyzers/undetectedSampler/partial-with-categories.csv", sep=";")
